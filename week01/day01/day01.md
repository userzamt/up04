...
[Пример создания кнопки запуска](https://redos.red-soft.ru/base/redos-8_0/8_0-administation/8_0-domain-redos/8_0-arm-msad/8_0-buttonshare/)

2. Используя **dnf** произвести поиск пакета по имени ***mysql***

1. Скачать метапакет **mysql84-community-release-el8-2.noarch.rpm**. Установить данный пакет с помощью **rpm**

[MySQL Community Downloads](https://dev.mysql.com/downloads/)

4. Используя **dnf** произвести поиск пакета по имени ***mysql-community-server***

>[!ERROR]
> На данном этапе будет ошибка чтения из репозиториев *MySQL*
> ![Ошибка чтения из репозиториев MySQL](./img/err01.png)
> Проблема чтения возникает из-за не правильного адреса ссылки в файлах репозиториев *MySQL*
> ![Битая ссылка](./img/err02.png)
> Ошибка возникает из-за неправильного указания версии ОС
> ![Корректная ссылка](./img/err03.png)
> В файлах репозиториев *MySQL*, переменная `$releasever` хранит некорректное значение
> ![Файлы репозиториев MySQL](./img/err04.png)


### bash-скрипт 3
В вашем репозитории, создать скрипт редактирующий файлы репозиториев *MySQL*, расположенные в директории `/etc/yum.repos.d/`. Файлы репозиториев *MySQL* имеют следующее названия:
- mysql-community.repo
- mysql-community-source.repo
- mysql-community-debuginfo.repo   

Необходимо отредактировать все ссылки в файлах репозитория *MySQL*, заменив `$releasever` на корректное значение версии ОС(см. изображения выше).

Для редактирования файлов использовать команду `sed`

Результат работы сохранить в истории изменений репозитория.

По завершению задания, создать снимок состояния виртуальной машины. Указать имя снимка - *yum.repos.d*. В описании к снимку, кратко перечислите проделанные Вами операции. 


### bash-скрипт 4
В вашем репозитории, написать bash-скрипт по поиску имен пакетов в репозиториях. Необходимо найти и вывести на экран список всех пакетов, хранящихся в репозиториях, имена которых начинаются на *mysql*. Скрипт должен выводить только названия пакетов. Имя каждого найденного пакета выводится на отдельной строке. Из всех найденных пакетов, при выводе на экран выделить другим цветом текста и фона пакет *mysql-community-server*.


### MySQL 1
Установить **mysql** как демон системы. Вывести информацию о состоянии работы демона **mysql**. Создать снимок состояния виртуальной машины. Указать имя снимка - *MySQL*. 

>[!CAUTION]
> Необходимо установить именно **mysql**. Не путать с пакетом СУБД **mariadb**

Используя терминал попытайтесь подключиться к СУБД MySQL.


### bash-скрипт 5
В вашем репозитории, создать bash скрипт, который выводит в терминал временный пароль учётной записи **root** СУБД MySQL, создаваемый при первом запуске. Пароль хранится в файле `/var/log/mysqld.log`. 

Пароль вывести следующим образом (см. пакеты установленные ранее)
![Вывод пароля совместно с cowsay -f dragon](./img/password.PNG)
...
>[!CAUTION]
> Для выполнения данного задания необходимо произвести проброс портов в VirtualBox
> ![Пример проброса портов](./img/forward.PNG)
> Для хост машины указать порт 3307, его использовать при подключение через *MySql Shell* и *MySql Workbench*. Порт на виртуальной машине стандартный для MySQL - 3306

4. Используя хост-машину и дамп базы данных(можно воспользоваться дампом из ПР27 по БД) восстановить БД. 
